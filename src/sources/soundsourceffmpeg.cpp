...
SoundSource::OpenResult SoundSourceFFmpeg::tryOpen(
    ...
    if (pDecoder->id == AV_CODEC_ID_AAC ||
        ...
        // Use the internal AAC decoder, independent of the order in which
        // FFmpeg registers decoders (libfdk_aac may precede it).
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(59, 0, 100) // FFmpeg 5.0
        const AVCodec* pInternalAacDecoder =
#else
        AVCodec* pInternalAacDecoder =
#endif
                avcodec_find_decoder_by_name("aac");
        if (pInternalAacDecoder) {
            pDecoder = pInternalAacDecoder;
        }
